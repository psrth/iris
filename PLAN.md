...
- **Phase 4 — readme + packaging.** goreleaser; `go install`; curl install script; brew (`iris-tl`) if warranted. Release. **Packaging done 2026-09-01**: `.goreleaser.yaml` (darwin/linux × amd64/arm64, tailcat's `build-tags.txt`, stripped, version injected), `ci.yml` (vet + race tests on PRs) and `release.yml` (goreleaser on `v*` tags), `install.sh` at the repo root (checksum-verified, `IRIS_VERSION`, `IRIS_INSTALL_DIR`), `iris -version`. First tag is `v1.0.0`. Brew deferred until a tap is worth a second repo. README still open.
- **Phase 5 — website.**
